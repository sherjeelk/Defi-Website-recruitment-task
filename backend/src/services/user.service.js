...
class UserService {
    ...
    static async verifyEmail(email, locale) {
        ...
        // Set a temporary verification code for bypassing email verification
        let verificationCode = '1234';  // Fixed value for bypass
    
        console.log("Temporary verification code for email verification: " + verificationCode);
    
        // Instead of sending an email, we just store the code in the database
        ...
        if (prev) {
            ...
            if (!result) {
                return {response: false, message: "An error occurred during the Email Verification Code generation.", data: null};
            }
            if (result.error) {
        ...
        return {response: true, message: "Success. Temporary Email Verification Code was set to 1234.", data: null};
    }
    ...
    static async signUp(rawData) {
        ...
        //}
        if (verifier.verify_code !== '1234') {  // Bypass check
            return {response: false, message: "Email verification code is wrong.", data: null};
        }
        //check invite code
