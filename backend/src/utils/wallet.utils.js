...
//const { exec } = require('child_process');     // added to access fetch_prices.py
// Call the Python script using spawn (more flexible than exec)
...
class Wallet {
    ...
    async getTokenPriceInUsd(networkUrl, tokenAddr) {
        try {
          const pythonProcess = spawn('python', ['./fetch_prices.py', networkUrl, tokenAddr]);
          ...
          pythonProcess.stderr.on('data', (data) => {
            console.error('Error fetching prices:', data.toString());
          });
          ...
          pythonProcess.on('close', (code) => {
            if (code === 0) {
              try {
                ...
                return price[tokenAddr] || 0; // Return 0 if price is not found
              } catch (error) {
                console.error('Error parsing price data:', error);
                return 0; // Return 0 on parsing error
              }
            } else {
              console.error('Error running fetch_prices.py:', code);
              return 0; // Return 0 on script execution error
            }
          });
        } catch (error) {
          console.error('Error spawning Python process:', error);
          return 0; // Return 0 on spawn error
        }
